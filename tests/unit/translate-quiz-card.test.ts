...
import {
  getCachedTranslatedQuizCard,
  translateQuizCard,
} from "@/lib/translation/translate-quiz-card";
...
describe("translateQuizCard", () => {
  it("translates public and answered-review prose while preserving artifact source", async () => {
    ...
    expect(result.review).toEqual({
      decisionCriteria: ["日本語訳: Route requests to a private upstream."],
      rationale: "日本語訳: The proxy owns the public request boundary.",
      choices: [
        {
          id: "a",
        ...
        {
          id: "b",
          explanation: "日本語訳: Compilation is a build concern.",
          consequence: "日本語訳: Requests are not routed.",
        },
      ],
      practicalNotes: ["日本語訳: Forward trusted client headers explicitly."],
      checkQuestion: "日本語訳: Where does TLS termination happen?",
      feedback: "日本語訳: Review the linked source.",
    });
  ...
  it("does not translate or return hidden review prose before an answer", async () => {
    ...
    expect(result.reviewStatus).toBe("hidden");
    expect(result).not.toHaveProperty("review");
    expect(result.choices).toEqual([{ id: "a", label: "日本語訳: Choice A" }]);
          ...
          choices: [
            {
              ...
              label: "Forwards client requests.",
              correct: true,
              explanation: "It owns the public boundary.",
              consequence: "The upstream remains private.",
            },
            {
              id: "b",
              label: "Compiles frontend assets.",
              correct: false,
              explanation: "Compilation is unrelated.",
              consequence: "Requests remain unrouted.",
            },
          ],
          decisionCriteria: ["Route requests before the upstream."],
          rationale: "The proxy is the public boundary.",
          practicalNotes: ["Forward trusted headers explicitly."],
          checkQuestion: "Where can TLS terminate?",
        },
        feedback: "Review the linked source.",
        revealReview: true,
      },
