...
import { podcastSettings } from "@/db/schema";
...
export async function GET(request: Request) {
  ...
  const secret = env.GOOGLE_OAUTH_CLIENT_SECRET_KEYCHAIN_SERVICE
    ? await clientSecret(env.GOOGLE_OAUTH_CLIENT_SECRET_KEYCHAIN_SERVICE, "GOOGLE_OAUTH_CLIENT_SECRET")()
    : undefined;
  ...
  await db.insert(podcastSettings).values({ userId: session.userId, includeCalendar: true }).onConflictDoUpdate({
    target: podcastSettings.userId,
    set: { includeCalendar: true },
  });
