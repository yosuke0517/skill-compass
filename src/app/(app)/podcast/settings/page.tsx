  ...
  return (
    <>
      ...
      <form action={savePodcastSettingsAction} className="podcast-settings-form">
        ...
        <section id="connections" className="management-card"><h2>Inputs & connections</h2><label className="podcast-toggle"><input name="useSources" type="checkbox" defaultChecked={data.settings.useSources} /><span><strong>Use Sources</strong><small>Use the selected sources below as briefing material.</small></span></label><label className="podcast-toggle"><input name="includeNews" type="checkbox" defaultChecked={data.settings.includeNews} /><span><strong>News</strong><small>Include current engineering news when available.</small></span></label><div className="podcast-connection-row"><label className="podcast-toggle"><input name="includeCalendar" type="checkbox" defaultChecked={connected.has("google-calendar") && data.settings.includeCalendar} disabled={!connected.has("google-calendar")} /><span><strong>Google Calendar</strong><small>{connected.has("google-calendar") ? "Connected. Read time and title by default." : "Connect to enable calendar input."}</small></span></label><Link href="/api/integrations/google/start" className="button-link">{connected.has("google-calendar") ? "Reconnect" : "Connect"}</Link></div><label className="podcast-toggle"><input name="includeXPublic" type="checkbox" defaultChecked={connected.has("x") && data.settings.includeXPublic} disabled={!connected.has("x")} /><span><strong>Public X posts</strong><small>{connected.has("x") ? "Use public posts as secondary context." : "Connect X to enable public posts."}</small></span></label><div className="podcast-connection-row"><label className="podcast-toggle"><input name="includeXPersonal" type="checkbox" defaultChecked={connected.has("x") && data.settings.includeXPersonal} disabled={!connected.has("x")} /><span><strong>Personal X timeline and bookmarks</strong><small>{connected.has("x") ? "Connected. Personal sources are available." : "Connect to enable your timeline and bookmarks."}</small></span></label><Link href="/api/integrations/x/start" className="button-link">{connected.has("x") ? "Reconnect" : "Connect"}</Link></div></section>
