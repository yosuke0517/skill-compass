...
export default async function PodcastPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  return (
    <>
      ...
      {history.episodes.length > 0 ? <section className="podcast-episode-list"><div className="management-card-heading"><h2>Recent episodes</h2><span>{history.jobs.length} jobs</span></div>{history.episodes.map((episode, index) => { const asset = history.assets.find((item) => item.episodeId === episode.id); const job = history.jobs.find((item) => item.episodeId === episode.id); return <article key={episode.id} className={index === 0 ? "podcast-current-episode" : undefined}><div data-podcast-playback-order><strong data-podcast-episode-title>{episode.title}</strong><small data-podcast-episode-state>{episode.language} · {episode.status}</small></div>{asset ? <div className="podcast-episode-actions"><PodcastPlayer src={`/api/podcast/assets/${asset.id}`} title={episode.title} /><a href={`/api/podcast/assets/${asset.id}?download=1`} download>Download</a><PodcastEpisodeChat episodeId={episode.id} title={episode.title} /></div> : job?.status === "failed" ? <form action={retryPodcastJobAction}><input type="hidden" name="jobId" value={job.id} /><button type="submit" className="text-button">Retry</button></form> : <span>{episode.sourceSnapshot.length} sources</span>}</article>; })}</section> : null}
