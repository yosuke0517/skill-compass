...
### Practical Today learning verification (2026-07-28)

- `0016_practical_user_scoped_today`の適用前に、repository外へtimestamp付きMySQL backupを作成した。migrationとseedは初回・冪等再実行とも成功した。
- active reviewed questionは70件、7カテゴリ各10件、active legacy questionは0件。learner-stateのowner NULLとscore/self-assessmentの所有者別重複は0件だった。
...
- Calendar / X入力はProvider境界を追加済み。OAuth接続が未設定の環境ではSources生成を壊さず空入力として扱う。
- OAuth state境界、Google Calendar / X OAuth 2.0の開始・Callback route、PKCE、暗号化Token保存を追加済み。実環境でのTODOはGoogle/Xのscope確認、Callback疎通、Calendar/X APIの実データ取得、Token refresh。
- News入力は`PODCAST_NEWS_FEED_URLS`のRSS/Atom URLを使う差し替え可能なProviderとして追加済み。未設定なら取得しない。
